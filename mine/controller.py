...
from Aboutme.settings import BASE_URL,SAE_DEFAULT_STORAGE_DOMAIN_NAME

online = os.environ.get("APP_NAME", "")
if online :
    import sae.storage
    import re
    import sae
    ...
    if config.Base64:
        ...
        try:
            if not online:
                ...
                truelyName = buildFileName(filename)
                domain_name = SAE_DEFAULT_STORAGE_DOMAIN_NAME
                bucket = sae.storage.Bucket(domain_name) #创建Bucket对象
                bucket.put_object(GetConfigValue("imageUrlPrefixlocal")+truelyName,buf.read())
...
def listFileManage(request, imageManagerListPath, imageManagerAllowFiles, listsize):
    ...
    localPath = BASE_URL + imageManagerListPath
    filelist = []
...
def uploadimageHandler(request):
    ...
    SavePath = GetConfigValue("imageUrlPrefixlocal")
